...
//
// DESCRIPTION:
// This file contains the main entry point for the 3D rendering application.
// It sets up the rendering environment using GLFW and GLEW, manages the 
// creation and configuration of the window, and orchestrates the rendering loop.
//
// FUNCTIONALITY:
// Initialize GLFW and GLEW libraries for OpenGL context and window management.
// Create and manage the main display window using the ViewManager class.
// Load and compile shaders using the ShaderManager class.
// Prepare and render 3D scenes using the SceneManager class.
// Implement the main rendering loop, handling events and buffer swapping.
//
//
// USAGE:
// This application initializes OpenGL, loads shaders, and renders a 3D scene
// within a window. It relies on the SceneManager, ViewManager, and ShaderManager
// classes for its core functionality. The main loop continues running until the
// user closes the application window.
// NOTES:
// Make sure to provide valid paths to the shader files and textures. This code 
// assumes the presence of certain GLSL shader files and image textures.
//
// /////////////////////////////////////////////////////////////////////////////
...
//The function initializes the GLFW libraryand sets some configuration hints.
//These operations are constant time operations, so the overall time complexity is O(1).
bool InitializeGLFW();
bool InitializeGLEW();
